...
my ( $file_config, $file_clusters ) = read_config_file($config_file);

# Cluster definitions from [cluster:NAME] and [clusters] sections (if any)
# get synthesized into the same form as the flat `clusters =` key so the
# existing parser handles them. Sections override the flat key.
if ( %$file_clusters ) {
    my @parts;
    for my $name ( sort keys %$file_clusters ) {
        my $urls = $file_clusters->{$name};
        next unless $urls && @$urls;
        push @parts, "$name=" . join( ',', @$urls );
    }
    $file_config->{clusters} = join( ';', @parts ) if @parts;
}
...
sub read_config_file {
    ...
    return ( $opts, \%clusters ) unless -f $path;
    ...
    if ( $@ || !$cg ) {
        ( my $err = $@ || 'unknown error' ) =~ s/\s+$//;
        warn "WARNING: Cannot parse config file $path: $err\n";
        return ( $opts, \%clusters );
    }

    my %raw     = $cg->getall;
    my $blocks  = delete $raw{cluster};

    # Top-level scalars become $opts (skip arrayref/hashref values).
    for my $k ( keys %raw ) {
        $opts->{$k} = $raw{$k} unless ref $raw{$k};
    }

    # <cluster NAME> blocks: each has zero or more `url` entries.
    if ( $blocks && ref $blocks eq 'HASH' ) {
        for my $name ( keys %$blocks ) {
            my $block = $blocks->{$name};
            next unless ref $block eq 'HASH';
            my $urls = $block->{url};
            next unless defined $urls;
            $urls = [$urls] unless ref $urls eq 'ARRAY';
            my @list = grep { length } map { s/^\s+|\s+$//gr } @$urls;
            $clusters{$name} = \@list if @list;
        }
    }
...
The config file uses Apache-style C<Config::General> syntax. Top-level
options are C<key = value> (or C<key value>), comments start with C<#>, and
clusters are defined as nested C<E<lt>cluster NAMEE<gt>> blocks with one
C<url> line per node.
...
Example F</etc/elastic-heap-monitor/elastic-heap-monitor.conf>:
    ...
    # Clusters — one block per cluster, one URL per line
    <cluster cluster1>
        url http://es1:9200
        url http://es2:9200
        url http://es3:9200
...
The legacy single-line C<clusters = c1=url1,url2;c2=url3> form is still
accepted.
...
C<name=url1,url2>:
    ...
    cluster1=http://es1:9200,http://es2:9200;cluster2=http://es3:9200
